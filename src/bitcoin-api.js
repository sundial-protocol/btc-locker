...
class BitcoinAPI {
  ...
  async getAddressInfo(address) {
    try {
      if (this.apiProvider === 'mempool' || this.apiProvider === 'blockstream') {
  ...
  async getAddressUtxos(address) {
    try {
      if (this.apiProvider === 'mempool' || this.apiProvider === 'blockstream') {
        ...
        const result = await this.makeRequest(`/addrs/${address}?unspentOnly=true&includeScript=true`);
        return result.txrefs?.map(utxo => ({
          txid: utxo.tx_hash,
